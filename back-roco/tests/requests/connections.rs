...
async fn stores_the_password_encrypted() {
    ...
    request::<App, _, _>(|request, ctx| async move {
        ...
        let row = back_roco::models::connections::Entity::find()
            .one(&ctx.db)
            .await
            .expect("consulta")
...
async fn removing_a_database_disables_it_instead_of_deleting_it() {
    ...
    request::<App, _, _>(|request, ctx| async move {
        ...
        let all = back_roco::models::connection_databases::Model::all_for(&ctx.db, id)
            .await
            .expect("consulta");
        assert_eq!(all.len(), 2);
        let disabled = all
...
async fn deletes_the_connection_and_its_databases() {
    request::<App, _, _>(|request, ctx| async move {
        ...
        let remaining = back_roco::models::connection_databases::Model::all_for(&ctx.db, id)
            .await
            .expect("consulta");
...
async fn testing_an_unreachable_host_records_the_failure() {
    ...
    request::<App, _, _>(|request, ctx| async move {
        ...
        let row = back_roco::models::connections::Model::find_one(&ctx.db, id)
            .await
            .expect("consulta")
            .expect("conexao");
