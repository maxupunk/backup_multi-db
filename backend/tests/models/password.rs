...
//! the legacy AdonisJS `config/hash.ts` configured it.
//!
...
use backend::models::password::PasswordHasher;
...
/// The fixture must describe the configuration the legacy AdonisJS
/// `config/hash.ts` declared. A
...
/// The expected value has to be derived by the legacy AdonisJS backend, which no
...
/// The reverse direction, for the shadow-traffic window when a
...
/// The end-to-end check can only run against the legacy AdonisJS backend.
#[test]
