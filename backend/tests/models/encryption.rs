...
//! AdonisJS `EncryptionService` itself. If this file fails, AES-256-GCM
//! byte-compatibility with the legacy backend is broken and every stored
//! credential is unreadable — it is a go/no-go gate, not a nice-to-have.
//!
//! The fixture is a recorded artifact: it can only be regenerated against a
//! running AdonisJS backend, which no longer lives in this repository.

use backend::models::encryption::EncryptionService;
...
/// as both run side by side during the shadow-traffic window.
///
