...
async fn volume_export_streams_a_gzip_when_docker_is_available() {
    ...
    let volume_name = format!("backend-test-export-{}", uuid::Uuid::new_v4());
...
async fn volume_backup_to_local_storage_works_when_docker_is_available() {
    ...
    let docker = bollard::Docker::connect_with_local_defaults().expect("docker client");
    let volume_name = format!("backend-test-backup-{}", uuid::Uuid::new_v4());
    ...
    request::<App, _, _>(move |request, ctx| async move {
        ...
        let settings =
            backend::initializers::settings::Settings::from_json(ctx.config.settings.as_ref())
                .expect("settings");
        let base = std::path::PathBuf::from(&settings.backup_storage_path);
        let full = backend::models::backup_storage::local_full_path(&base, relative_path)
        ...
        let _ =
            backend::models::storage_destinations::Model::delete_by_id(&ctx.db, storage_id).await;
    })
    .await;
...
async fn volume_remove_reports_conflict_when_in_use() {
    ...
    let docker = bollard::Docker::connect_with_local_defaults().expect("docker client");
    let volume_name = format!("backend-test-inuse-{}", uuid::Uuid::new_v4());
    let container_name = format!("backend-test-inuse-container-{}", uuid::Uuid::new_v4());
