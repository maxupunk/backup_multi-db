    ...
    request::<App, _, _>(|request, ctx| async move {
        ...
        let settings =
            backend::initializers::settings::Settings::from_json(ctx.config.settings.as_ref())
                .expect("settings");
        let encryption =
            backend::models::backup_runner::encryption_service(&settings).expect("cripto");
...
async fn insert_backup_at(
    ...
    destination_id: i64,
) -> backend::models::_entities::backups::Model {
        ...
        for (storage, key, browse_path) in [
            ...
            let settings =
                backend::initializers::settings::Settings::from_json(ctx.config.settings.as_ref())
                    .expect("settings");
            let encryption =
                backend::models::backup_runner::encryption_service(&settings).expect("encryption");
            let destination = backend::models::storage_destinations::Model::find_one(&ctx.db, id)
                .await
                .expect("consulta storage")
                .expect("storage existe");
            let (_, adapter) = backend::models::storage::explorer::open(
                &destination,
                &encryption,
