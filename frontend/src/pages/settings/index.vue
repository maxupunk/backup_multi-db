    ...
    <v-row>
          ...
          <v-card-text>
            <v-list>
              <v-list-item>
                ...
                <template #append>
                  <v-btn-toggle v-model="settings.theme" density="compact" mandatory rounded="lg"
      ...
      <v-col cols="12" md="6">
        <v-card class="mb-4">
          ...
          <v-card-text>
            <v-text-field v-model="destinationFilters.search" class="mb-4" clearable density="comfortable" hide-details
              label="Buscar destino" prepend-inner-icon="mdi-magnify" variant="outlined"
              @update:model-value="debouncedLoadDestinations" />

            <v-data-table class="elevation-0" :headers="destinationHeaders" :items="storageDestinations"
              :items-per-page="5" :loading="loadingDestinations" :mobile="!mdAndUp">
              ...
              <template #item.actions="{ item }">
                ...
                <v-menu v-else location="bottom end">
                  ...
                  <v-list density="compact">
                    ...
                    <v-list-item prepend-icon="mdi-delete" title="Excluir" @click="confirmDeleteDestination(item)" />
                  </v-list>
                </v-menu>
              </template>
        ...
        <v-card>
          ...
          <v-card-text>
            ...
            <v-btn class="mr-2" href="https://github.com" prepend-icon="mdi-github" target="_blank" variant="outlined">
              GitHub
            </v-btn>
      ...
      <v-card>
        ...
        <v-card-text>
          <v-form ref="destinationFormRef" @submit.prevent="saveDestination">
            <v-row>
              <v-col cols="12" sm="8">
                <v-text-field v-model="destinationForm.name" label="Nome *" prepend-inner-icon="mdi-label"
                  :rules="[rules.required]" />
              </v-col>

              <v-col cols="12" sm="4">
                <v-select v-model="destinationForm.type" :items="destinationTypes" label="Tipo *"
                  prepend-inner-icon="mdi-shape" :rules="[rules.required]"
                  @update:model-value="onDestinationTypeChange" />
              </v-col>

              <v-col cols="12" sm="6">
                <v-select v-model="destinationForm.status" :items="destinationStatusOptions" label="Status"
            ...
            <template v-if="destinationForm.type === 'local'">
              ...
              <v-alert v-if="destinationConfig.basePath && destinationConfig.basePath.trim() !== '/app/storage/backups'"
                ...
                <small class="text-medium-emphasis">
                  Em caso de dúvida, mantenha o caminho padrão.
                </small>
              </v-alert>
            </template>
            ...
            <template v-else-if="destinationForm.type === 's3'">
              <v-row>
                <v-col cols="12" sm="6">
                  <v-text-field v-model="destinationConfig.region" label="Região *" prepend-inner-icon="mdi-earth"
                    :rules="[rules.required]" />
                </v-col>
                <v-col cols="12" sm="6">
                  <v-text-field v-model="destinationConfig.bucket" label="Bucket *" prepend-inner-icon="mdi-bucket"
                    :rules="[rules.required]" />
                </v-col>
                <v-col cols="12" sm="6">
                  <v-text-field v-model="destinationConfig.accessKeyId" label="Access Key ID *"
                    prepend-inner-icon="mdi-key" :rules="[rules.required]" />
                </v-col>
                <v-col cols="12" sm="6">
                  <v-text-field v-model="destinationConfig.secretAccessKey" label="Secret Access Key *"
                    prepend-inner-icon="mdi-lock" :rules="[rules.required]" type="password" />
                </v-col>
                <v-col cols="12" sm="6">
                  <v-text-field v-model="destinationConfig.endpoint" label="Endpoint"
                    prepend-inner-icon="mdi-link-variant" />
                </v-col>
                <v-col cols="12" sm="6">
                  <v-text-field v-model="destinationConfig.prefix" label="Prefix"
                    prepend-inner-icon="mdi-folder-outline" />
                </v-col>
                <v-col cols="12">
                  <v-switch v-model="destinationConfig.forcePathStyle" color="primary" hide-details
            ...
            <template v-else-if="destinationForm.type === 'gcs'">
              <v-row>
                ...
                <v-col cols="12" sm="6">
                  <v-text-field v-model="destinationConfig.projectId" label="Project ID"
                    prepend-inner-icon="mdi-identifier" />
                </v-col>
                <v-col cols="12">
                  <v-textarea v-model="destinationConfig.credentialsJson" auto-grow label="Credentials JSON"
                    prepend-inner-icon="mdi-file-code" rows="3" />
                </v-col>
                <v-col cols="12" sm="6">
                  <v-text-field v-model="destinationConfig.prefix" label="Prefix"
                    prepend-inner-icon="mdi-folder-outline" />
                ...
                <v-col cols="12" sm="6">
                  <v-switch v-model="destinationConfig.usingUniformAcl" color="primary" hide-details
            ...
            <template v-else-if="destinationForm.type === 'azure_blob'">
              <v-row>
                <v-col cols="12" sm="6">
                  <v-text-field v-model="destinationConfig.container" label="Container *"
                ...
                <v-col cols="12">
                  <v-textarea v-model="destinationConfig.connectionString" auto-grow label="Connection String *"
                    prepend-inner-icon="mdi-lock" :rules="[rules.required]" rows="3" />
                </v-col>
              </v-row>
            </template>
            ...
            <template v-else-if="destinationForm.type === 'sftp'">
              <v-row>
                <v-col cols="12" sm="6">
                  <v-text-field v-model="destinationConfig.host" label="Host *" prepend-inner-icon="mdi-server"
                    :rules="[rules.required]" />
                </v-col>
                <v-col cols="12" sm="6">
                  <v-text-field v-model.number="destinationConfig.port" label="Porta" prepend-inner-icon="mdi-ethernet"
                    :rules="[rules.portOptional]" type="number" />
                </v-col>
                <v-col cols="12" sm="6">
                  <v-text-field v-model="destinationConfig.username" label="Usuário *" prepend-inner-icon="mdi-account"
                    :rules="[rules.required]" />
                </v-col>
                <v-col cols="12" sm="6">
                  <v-text-field v-model="destinationConfig.password" label="Senha" prepend-inner-icon="mdi-lock"
                    type="password" />
                </v-col>
                <v-col cols="12">
                  <v-textarea v-model="destinationConfig.privateKey" auto-grow label="Private Key"
                    prepend-inner-icon="mdi-key" rows="3" />
                </v-col>
                <v-col cols="12" sm="6">
                  <v-text-field v-model="destinationConfig.passphrase" label="Passphrase"
